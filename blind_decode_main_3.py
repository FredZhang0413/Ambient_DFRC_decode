...
def build_tag_codebook(L: int, U: int, device: torch.device) -> Tensor:
    ...
    Build U codewords x in C^L with the following properties:
      * Each x is orthogonal to 1_L (x ⊥ 1)
      * ||x||^2 = L
      * When U ≤ L-1: codewords are mutually orthogonal (x_i ⊥ x_j for i≠j)
      * When U > L-1: first L-1 codewords are orthogonal, remaining ones only satisfy x ⊥ 1
    ...
    print(f"\n🔧 Generating {U} tag codewords (L={L}):")
    print(f"  - Phase 1: {num_orthogonal} strictly orthogonal codewords")
    if U > L - 1:
        print(f"  - Phase 2: {U - num_orthogonal} additional codewords (x ⊥ 1 only)")
    ...
    # Phase 2: Generate additional codewords (only x ⊥ 1, not mutually orthogonal)
    if U > L - 1:
        ...
        for u_idx in range(num_orthogonal, U):
            ...
            if best_v is not None:
                ...
                print(f"  ✓ Generated codeword {u_idx + 1}/{U}, max_corr={best_max_corr:.3f}")
            else:
    ...
    print(f"  - Max |<1, x_i>|/L = {max_dc:.6f} (should be ≈ 0)")
    
    # 2. Check norms
    norms = torch.sqrt((X.conj() * X).real.sum(dim=0))
    max_norm_err = torch.max(torch.abs(norms - math.sqrt(L))).item()
    print(f"  - Max ||x_i||² error = {max_norm_err**2:.6f} (should be ≈ 0, target = {L})")
    ...
    print(f"  - Max |<x_i, x_j>|/L = {max_corr:.3f} (i≠j)")
    print(f"  - Mean|<x_i, x_j>|/L = {mean_corr:.3f} (i≠j)")
...
def main():
    ...
    # System parameters
    L = 10  # PRI number per frame (reduced)
    Np = 31
    Qmin, Qmax = 10, 20
    Ma = 16
    U = 16  # Tag codebook size (U > L-1 = 9)
    P_SR = 1.0
    P_STR = 1.0
    Q = Qmax - Qmin
    K = Np + Q
